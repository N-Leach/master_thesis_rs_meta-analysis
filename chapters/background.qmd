...
*Note:* Adapted from @un-ggim2019 and @haddaway_evidence_2022.
...
::: {#tbl-tex style="text-align: justify"}
```{r}
data <- data.frame(
...
*Note:* Confusion matrix for a classification with four classes, where the rows ($r$) represent the reference (observed) classification and the columns ($c$) represent the predicted classes. $m_{rc}$ is the number of instances predicted in reference class $r$ and predicted class $c$, and $n$ is the total number of instances (i.e. the number of pixels/objects classified).

Confusion matrix of four classes
:::

::: {style="text-align: justify"}
From this matrix, performance measures such as overall accuracy are derived [@fao2016; @un-ggim2019; @stehman2019]. Where the overall accuracy is the total number of successful classifications, $s$ over total number of instances, $n$.

$$
\text{Overall Accuracy (OA)} = \frac{\sum^q_{r=1}m_{rr}}{n}= \frac{s}{n}
$$ {#eq-OA}
...
To illustrate how remote sensing data and machine leaning can be used to support ecological sustainable development, @owers2022 developed an approach to monitor and map land cover across Australia using techniques.

The study utilized Landsat sensor data archive through Digital Earth Australia to generate annual land cover maps from 1988 to 2020 at a 25-meter resolution. The study used random forest and artificial neural networks to classify individual pixels according to the FAO's Land Cover Classification System (LCCS) framework.
:::
